# NOTE: cyberrunner_state_estimation/cyberrunner_state_estimation/utils/anim_3d.py
import math
...
class Anim3d:
    ...
    def init_3d_anim(self, T__W_C):
        ...
        print("init done. background saved")
...
if __name__ == "__main__":
    # Create an Anim3d instance with desired view limits
    anim = Anim3d(xlim=[-0.02, 0.33], ylim=[-0.04, 0.31], zlim=[0.0, 0.21])

    # Initialize the 3D animation with camera transformation matrix
    T__W_C = np.eye(4)  # Example: identity transformation
    anim.init_3d_anim(T__W_C)

    num_frames = 100  # Number of frames to
    
    # Now you can update the animation in a loop
    for i in range(num_frames):
        # Update ball position, maze corners, etc.
        anim.B__W = 1.0 * np.random.rand(3)
        anim.maze_corners__W = 0.3 * np.random.rand(4, 3)

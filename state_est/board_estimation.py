...
class EstimationPipeline:
    def __init__(
        ...
        if estimator == "FiniteDiff":
            ...
            raise ValueError(f"Estimator {estimator} not implemented")
    ...
    def estimate(self, frame, return_ball_subimg=False):
        ...
        Args:
            frame: np.ndarray, dim: (height,width) # NOTE: this takes [height, width]
        ...
        else:  # type(self.estimator).__name__ == "KF": # FiniteDiff
            alpha_est = inputs[0]
            beta_est = inputs[1]
        alpha_est *= 180 / np.pi
        beta_est *= 180 / np.pi
        estimator = time.time() - t0

...
class EstimationPipeline:
    def __init__(self, 
        ...
        #share = get_package_share_directory('brio_state_estimation')
        markers = np.loadtxt("markers.csv", delimiter=',', skiprows=1, usecols=[2, 3])
        # print("LOADED MARKERS:", markers)
    ...
    def estimate(self, frame, return_ball_subimg=False):
        ...
        tmeas = time.time()-t0
        t0 = time.time()
        x_hat, P = self.estimator.estimate(inputs = inputs, 
                                        measurement = np.array([xb, yb]))
        ...
        if self.show_image:
            ...
            if np.isnan(xb) or np.isnan(yb):
                ...
                xb = frame.shape[1] // 2
                yb = frame.shape[0] // 2
                cv2.circle(frame, (int(xb), int(yb)), 10, (0, 255, 0), -1)
            cv2.imshow("ori", frame)
            cv2.waitKey(1)
        # print(" NOTE THE SIGNATURE OF THE ESTIMATOR PIPLINE DEPENDS OF return_ball_subimg", return_ball_subimg)
        if return_ball_subimg:
